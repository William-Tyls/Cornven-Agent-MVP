...
export class OpenAiEmbeddingProvider implements EmbeddingProvider {
  ...
  async embed(texts: string[]): Promise<number[][]> {
    ...
    try {
      ...
      throw new Error('Embedding provider request failed before receiving a response.');
    }
    if (!response.ok) {
    ...
    return entries.map((item, expectedIndex) => {
      if (item.index !== expectedIndex || item.embedding.length !== this.dimensions) {
        throw new Error(
          'Embedding provider returned an unexpected vector index, count, or dimension.',
        );
      }
      if (item.embedding.every((value) => value === 0)) {
        throw new Error('Embedding provider returned an unusable zero vector.');
      }
...
export function createEmbeddingProvider(
  ...
  if (provider === 'openai') {
    return new OpenAiEmbeddingProvider({
      apiKey: environment.RAG_EMBEDDING_API_KEY ?? '',
      dimensions,
